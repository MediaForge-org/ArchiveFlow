# Prompt P060 — Source SDK, Adapter Registry, Capability UI und Importer
...
**Phase:** `06-advanced-v1x`  
**Abhängigkeiten:** P002, P008, P040, P057  
**UI-Referenzen:** UI-093, UI-094, UI-095, UI-104, UI-123
**Empfohlener Branch:** `pkg/P060-erweiterbare-source-adapter-architektur-und-impo`

## Auftrag

Bereite die Architektur darauf vor, später weitere Quellen oder vorhandene Archive zu importieren.
...
- Das in P001/P002 angelegte Source SDK produktionsreif ausbauen; **kein nachträgliches Herauslösen von Instagram-Code** zulassen.
- Source Registry/Manifest, versionierte `adapterApiVersion`, Capability-Descriptor, Auth/Session-State, Pagination, Error Mapping, Source Rate Policy und Health-API vervollständigen.
- UI Source Manager: installierte/integrierte Sources, Version, Connection/Auth State, Capabilities und Health anzeigen.
- Importer als eigene Source-/Importer-Plugins behandeln: bestehende lokale Ordner, Instagram Data Export ZIP/ähnliche Exporte, JSON/CSV/Manifest, ArchiveFlow Portable Package.
- Namespaced Source-Metadaten und optionale Raw-Payload-Referenzen validieren.
- Einen zweiten lokalen Dummy-/Fixture-Source-Adapter registrieren und beweisen, dass Core, Task Engine, Library, Search und Storage nicht geändert werden müssen.
...
- Verwende die referenzierten Mockups (UI-060) als visuelle Zielrichtung, sofern vorhanden.
...
- [ ] Neuer Dummy-Adapter kann ausschließlich über Registry/Manifest registriert werden.
- [ ] Keine Änderung an Task Engine/Library/Storage nötig, um Dummy-Content anzuzeigen und zu archivieren.
- [ ] Capability `unsupported` blendet nicht verfügbare Source-Funktionen korrekt aus.
- [ ] Adapter-Version/Health wird im Source Manager angezeigt.
- [ ] Import verändert Originaldateien nicht.
- [ ] Keine ausführbaren Remote-Code-Plugins in der Browser-Extension.
...
6. Ob P061 bzw. das nächste abhängige Paket sicher gestartet werden kann.
