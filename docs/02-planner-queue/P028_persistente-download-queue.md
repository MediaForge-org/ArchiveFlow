# Prompt P028 — Persistente Task Engine und Download Queue
...
**Phase:** `02-planner-queue`  
**Abhängigkeiten:** P006, P023, P027  
**UI-Referenzen:** UI-100, UI-101, UI-120
**Empfohlener Branch:** `pkg/P028-persistente-download-queue`

## Auftrag

Speichere Download-Jobs persistent und unabhängig vom UI-Lifecycle.
...
- Die bisher geplante Download Queue als **generische persistente Task Engine** implementieren; Download bleibt ein Task-Typ.
- Task-Typen/Handler-Registry für mindestens DISCOVER_ACCOUNT, DISCOVER_CONTENT, RESOLVE_MEDIA, DOWNLOAD_MEDIA, VERIFY_MEDIA, HASH_MEDIA, GENERATE_THUMBNAIL, SYNC_ACCOUNT, REINDEX_ARCHIVE vorbereiten.
- `task_dependencies`/DAG unterstützen: Task läuft erst, wenn Dependencies erfüllt sind; Failure/Cancel-Propagation klar definieren.
- Idempotency Keys verhindern doppelte aktive Tasks für dieselbe fachliche Operation.
- Status, Fortschritt, Attempts, SourceId, AccountId, Content/Media-Bezug, Priorität, Zeitstempel und Resume-Metadaten persistent speichern.
- Source-spezifische Ausführung wird über Handler/Adapter/Policy injiziert; Task Engine kennt keine Instagram-Endpunkte.
- UI darf weiterhin eine „Download Queue“-Ansicht anbieten, filtert dann Download-bezogene Tasks aus der generischen Engine.
...
- Verwende die referenzierten Mockups (UI-011) als visuelle Zielrichtung, sofern vorhanden.
...
- [ ] App-Neustart verliert keine Tasks/Dependencies.
- [ ] Ein Medium hat keine zwei gleichzeitig aktiven identischen Download-Tasks.
- [ ] Ein Thumbnail-Fehler erzwingt keinen erneuten Download eines bereits verifizierten Blobs.
- [ ] Dummy-Source-Tasks laufen durch dieselbe Engine wie Instagram.
- [ ] Statuswechsel sind auditierbar.
...
6. Ob P029 bzw. das nächste abhängige Paket sicher gestartet werden kann.
