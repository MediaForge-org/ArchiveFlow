# Prompt P006 — SQLite Schema, Repository Layer und Migrationen
...
**Phase:** `00-foundation`  
**Abhängigkeiten:** P002, P003  
**UI-Referenzen:** UI-106, UI-107, UI-108
**Empfohlener Branch:** `pkg/P006-sqlite-schema-repository-layer-und-migrationen`

## Auftrag

Lege die persistente Datenbasis für Accounts, Medien, Varianten, Archive, Jobs, Syncs und Einstellungen an.
...
- Normalisiertes source-neutrales Schema mit Tabellen/Repositories mindestens für: `sources`, `source_accounts`, `archive_identities`, `identity_links`, `content_items`, `media_references`, `media_assets`, `physical_blobs`, `blob_locations`, `relationships`, `tasks`, `task_dependencies`, `sync_runs`, `metadata_core`, `metadata_source`, `metadata_user`, `provenance`, `audit_events`.
- Keine Tabellen `instagram_posts`, `instagram_reels` usw. Instagram-spezifische Zusatzdaten nur namespaced in Source-Metadaten/Adaptertabellen, falls technisch zwingend und isoliert.
- Interne ArchiveFlow-IDs als Primärschlüssel; `(source_id, external_id)` nur unique/indexed secondary identity.
- Dateipfade ausschließlich als Storage-Locations; Pfadänderung darf logische Identität nicht ändern.
- Indizes für Source+ExternalID, Account, Publish-Datum, ContentKind, Media-Hash, Taskstatus, Provenance und Suchindex-Verknüpfung.
- SQLite enthält keine großen Mediendateien oder riesigen Raw-Payload-Blobs; große optionale Raw-Responses werden dateibasiert/komprimiert referenziert.
- Migrationssystem mit sicherer Forward-only Strategie oder getesteter Up/Down-Strategie; Daten müssen erhalten bleiben.
- Repository-Interfaces source-neutral; keine SQL-Strings im UI.
- Testdatenbank mit mindestens zwei Dummy-Sources, um Source-Hardcodes früh zu erkennen.
...
- [ ] Frische DB migriert vollständig.
- [ ] Zwei Sources mit gleichlautender externer Account-/Media-ID kollidieren nicht.
- [ ] Pfadänderung eines Blobs verändert MediaAsset/Content-ID nicht.
- [ ] Mehrere MediaReferences dürfen denselben PhysicalBlob referenzieren.
- [ ] Keine Instagram-spezifische Core-Tabelle.
- [ ] Relevante Queries für große Archive besitzen Indizes und Integrationtests.
...
6. Ob P007 bzw. das nächste abhängige Paket sicher gestartet werden kann.
