# Prompt P009 — Test-Fundament, Source Contract Tests und Instagram-Fixtures
...
**Phase:** `00-foundation`  
**Abhängigkeiten:** P001, P002, P004  
**UI-Referenzen:** UI-095, UI-104
**Empfohlener Branch:** `pkg/P009-test-fundament-und-instagram-fixtures`

## Auftrag

Baue reproduzierbare Tests ohne dauernd live gegen Instagram zu testen.
...
- Gemeinsames Test-Fundament für Core und alle zukünftigen Sources erstellen.
- Source Contract Test Suite definieren, die jeder Adapter ausführen kann: stabile IDs, Pagination/Cursor, Timestamp-Normalisierung, Capability-Konsistenz, Media-Candidate-Reihenfolge, Cancellation, Error Mapping, Retry-Sicherheit, keine Core-Leaks.
- `tests/fixtures/<source>/...` Struktur etablieren. Instagram erhält als Source #1 Fixtures für Profil, Bildpost, Video/Reel, Carousel, Story, Highlight, Saved, Following und Fehlerfälle.
- Einen minimalen Dummy-/Reference-Adapter nur für Tests implementieren, der eine zweite Source simuliert und die Contract Suite besteht.
- Tests dürfen standardmäßig kein Live-Netzwerk benötigen; Live-Smokes explizit getrennt/opt-in.
- Fixture-Sanitizer/Validator sicherstellen: keine Cookies, Tokens, Passwörter oder private Header einchecken.
- Regressionstests für Capability `unsupported` statt Fehler.
...
- [ ] Core-Testlauf benötigt kein Instagram-Netzwerk.
- [ ] Instagram- und Dummy-Adapter laufen durch dieselbe Contract Suite.
- [ ] Dummy-Adapter kann registriert werden, ohne Core/Testcode zu forken.
- [ ] Fixtures enthalten keine Secrets.
- [ ] Mindestens ein Pagination-, Cancellation-, Rate-Limit- und malformed-payload-Negativtest vorhanden.
...
6. Ob P010 bzw. das nächste abhängige Paket sicher gestartet werden kann.
