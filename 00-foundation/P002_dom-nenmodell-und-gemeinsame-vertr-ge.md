# Prompt P002 — Source-neutrales Domänenmodell, Media Identity und gemeinsame Verträge
...
**Phase:** `00-foundation`  
**Abhängigkeiten:** P001  
**UI-Referenzen:** UI-095, UI-106, UI-107, UI-108, UI-110
**Empfohlener Branch:** `pkg/P002-dom-nenmodell-und-gemeinsame-vertr-ge`

## Auftrag

Definiere stabile, serialisierbare Kernmodelle für Accounts, Media, Carousel-Elemente, Download-Jobs, Archive und Fehler.
...
- Source-neutrale TypeScript- und Rust-Modelle fachlich spiegeln und Roundtrip-serialisierbar machen.
- Eigene ArchiveFlow-ID-Typen definieren (`SourceId`, `SourceAccountId`, `ArchiveIdentityId`, `ContentItemId`, `MediaReferenceId`, `MediaAssetId`, `PhysicalBlobId`, `TaskJobId`).
- Externe Source-IDs niemals als interne PK verwenden.
- Kernmodelle: `SourceDescriptor`, `SourceCapabilities`, `SourceAccount`, `ArchiveIdentity`, `ContentItem`, `MediaReference`, `MediaAsset`, `PhysicalBlob`, `Relationship`, `MediaCandidate`, `ArchiveLocation`, `TaskJob`, `TaskDependency`, `SyncDiff`.
- `ContentKind` source-neutral definieren (z. B. image_post, video_post, short_video, ephemeral, gallery, article, comment, other); Source-spezifische Begriffe gehören in `source_metadata`.
- Metadaten-Schichten explizit modellieren: Core, Source-Namespace, optional Raw Source Payload Reference, User Metadata.
- Provenance-Struktur definieren: Source, externe IDs, Publish/Discovery/Download-Zeit, Adapter-Version, Candidate, Hash/Blob-Zuordnung.
- Multi-Asset/Gallery über geordnete `MediaReference[]` modellieren; gemischte Bild-/Videoelemente unterstützen.
- Statusmodell für Content/Media/Task getrennt halten; Downloadstatus nicht als allgemeiner Contentstatus missbrauchen.
- Capability-Descriptor granular und erweiterbar definieren, ohne zentrale Enum-Migration für jede neue Source-Funktion zu erzwingen.
- Versionierte IPC-DTOs in `packages/protocol`; keine Source-Klassen direkt über IPC serialisieren.
...
- [ ] Kein `InstagramAccount`, `InstagramPost` oder `Reel` als Core-Domänentyp.
- [ ] Account-ID ist getrennt von Username und externer Source-ID.
- [ ] Ein ContentItem kann 1..n MediaReferences mit stabiler Reihenfolge besitzen.
- [ ] Zwei Source-Referenzen können auf dasselbe MediaAsset/PhysicalBlob zeigen.
- [ ] Source-/Core-/User-Metadaten sind getrennt.
- [ ] Roundtrip-Tests decken alle öffentlichen DTOs ab.
...
6. Ob P003 bzw. das nächste abhängige Paket sicher gestartet werden kann.
